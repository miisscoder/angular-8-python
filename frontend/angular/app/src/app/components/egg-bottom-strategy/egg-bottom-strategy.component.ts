...
@Component({
  selector: 'egg-bottom-strategy-component',
  templateUrl: './egg-bottom-strategy.component.html',
  styleUrls: ['./egg-bottom-strategy.component.scss']
})
export class EggBottomStrategyComponent implements OnInit, OnChanges {
  ...
  strategy = 0;
  strategies = ['box', 'bottom', 'everyday', 'everymonday'];
  rateMaxLoss = 0;
  ...
  chart() {
    ...
    if (this.dataStock && this.dataStock.length > 0) {
      ...
      this.svg = d3.select('#chart').append('svg').attr('viewBox', [0, 0, this.width, this.height])
      this.gMainCharts = this.svg.append('g').attr('id', 'gMainCharts')
        .on('click', () => {
  ...
  unionData() {
    ...
    for (var i = keys.length - 1; i >= 0; i--) {
      if (this.contracts[keys[i]].checked) {
        ...
        _.sortBy(this.dataStock, ['date']);
      }
    }
  }

  detailLine(x) {
    this.svg.select('g#gLineFocus').remove();
    const g = this.svg.append('g').attr('id', 'gLineFocus');
  ...
  xAxisMainChart() {
    ...
    // svg
    this.svg.select('#gXDateChartAxis').remove();
    var g = this.svg.append('g').attr('id', 'gXDateChartAxis');
  ...
  RoomBar() {
    ...
    const leftYearRoom = this.svg.append('g').attr('class', 'leftRoomG')
      .attr('transform', 'translate(' + this.margin.left + ',0)')
      .call(
        d3.drag().on('drag', () => {
          const { x } = d3.event;
          d3.select('g.leftRoomG').attr('transform', 'translate(' +
            this.rangeControl(x) + ',0)').style('cursor', 'pointer');
          const dt = this.xRoom.invert(x);
          d3.select('g.leftRoomG').selectAll('text').text(dt);
    ...
    const rightYearRoom = this.svg.append('g').attr('class', 'rightRoomG')
      .attr('transform', 'translate(' + (this.width - this.margin.right) + ',0)')
      .call(
        d3.drag().on('drag', () => {
          const { x } = d3.event;
          d3.select('g.rightRoomG').attr('transform', 'translate(' +
            this.rangeControl(x) + ',0)')
            .style('cursor', 'pointer');
          const dt = this.xRoom.invert(x);
          d3.select('g.rightRoomG').selectAll('text').text(dt);
          this.endRoomTime = dt;
        }).on('end', () => {
          this.updateAfterRoom();
        })
  ...
  chartTimeProfit(bottom, top, beRoomChart) {
    ...
    var g = null;
    if (!beRoomChart) {
      this.gMainCharts.select('#gTimeProfitChart').remove();
      g = this.gMainCharts.append('g').attr('id', 'gTimeProfitChart');
      g.append('g').call(yTimeProfitAxis);
    } else {
      this.svg.select('#gTimeProfitChartRoom').remove();
      g = this.svg.append('g').attr('id', 'gTimeProfitChartRoom');
    }
  ...
  chartStockLine(bottom, top, beRoomChart) {
    ...
    this.yStock = d3.scaleLinear()
      .domain(d3.extent(this.dataStock, d => d.open)).nice()
      .range([bottom, top]);
    ...
    var g = null;
    if (!beRoomChart) {
      this.gMainCharts.select('#gStockChart').remove();
      g = this.gMainCharts.append('g').attr('id', 'gStockChart');
      g.append('g').call(yStockAxis);
    } else {
      this.svg.select('#gStockChartRoom').remove();
      g = this.svg.append('g').attr('id', 'gStockChartRoom');
    }
    // stock line , mean
  ...
  chartMaxBottom(bottom, top, beRoomChart) {
    ...
    var g = null;
    if (!beRoomChart) {
      this.gMainCharts.select('#gMaxBottomChart').remove();
      g = this.gMainCharts.append('g').attr('id', 'gMaxBottomChart');
      g.append('g').call(yMaxBottomAxis);
    } else {
      this.svg.select('#gMaxBottomChartRoom').remove();
      g = this.svg.append('g').attr('id', 'gMaxBottomChartRoom');
    }
  ...
  chartStackDeep(bottom, top, beRoomChart) {
    ...
    var g = null;
    if (beRoomChart) {
      this.gMainCharts.select('#gStackDeepChart').remove();
      g = this.gMainCharts.append('g').attr('id', 'gStackDeepChart');
    } else {
      this.svg.select('#gStackDeepChartRoom').remove();
      g = this.svg.append('g').attr('id', 'gStackDeepChartRoom');
  ...
  chartProfit(bottom, top, beRoomChart) {
    ...
    var g = null;
    if (!beRoomChart) {
      this.gMainCharts.select('#gProfitChart').remove();
      g = this.gMainCharts.append('g').attr('id', 'gProfitChart');
      g.append('g').call(yProfitAxis);
    } else {
      this.svg.select('#gProfitChartRoom').remove();
      g = this.svg.append('g').attr('id', 'gProfitChartRoom');
    }
  ...
  chartNetValue(bottom, top, beRoomChart) {
    ...
    var g = null;
    if (!beRoomChart) {
      this.gMainCharts.select('#gNetValueChart').remove();
      g = this.gMainCharts.append('g').attr('id', 'gNetValueChart');
      g.append('g').call(yNetValueAxis);
    } else {
      this.svg.select('#gNetValueChartRoom').remove();
      g = this.svg.append('g').attr('id', 'gNetValueChartRoom');
    }
    // net value line 
  ...
  chartValueDistrabution(bottom, top, beRoomChart) {
    ...
    var g = null;
    if (!beRoomChart) {
      this.gMainCharts.select('#gValueDistributionChart').remove();
      g = this.gMainCharts.append('g').attr('id', 'gValueDistributionChart');
      g.append('g').call(yValueDistributionAxis);
    } else {
      this.gMainCharts.select('#gValueDistributionChartRoom').remove();
      g = this.gMainCharts.append('g').attr('id', 'gValueDistributionChartRoom');
    }
